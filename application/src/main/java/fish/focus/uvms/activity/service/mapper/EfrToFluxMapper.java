...
import fish.focus.uvms.activity.fa.entities.FishingGearRoleEntity;
...
import fish.focus.uvms.activity.fa.utils.UnitCodeEnum;
import fish.focus.uvms.activity.model.efr.activities.BaseEfrActivity;
...
public class EfrToFluxMapper {
    ...
    private Set<FaCatchEntity> getCatchesForPriorNotificationOfArrival(PriorNotificationEfrActivity efrActivity) {
        ...
        Set<fish.focus.uvms.activity.fa.entities.FaCatchEntity> result = new HashSet<>();
        for (fish.focus.uvms.activity.model.efr.activities.PriorNotificationEstimatedCatch efrCatch : efrActivity.getEstimatedCatches()) {
            fish.focus.uvms.activity.fa.entities.FaCatchEntity fluxCatch = new fish.focus.uvms.activity.fa.entities.FaCatchEntity();
            fluxCatch.setTypeCode("ONBOARD");
            fluxCatch.setTypeCodeListId("FA_CATCH_TYPE");
            fluxCatch.setSpeciesCode(efrCatch.getCode()); // TODO double check that this is in fact in FAO format
            fluxCatch.setSpeciesCodeListid("FAO_SPECIES");
